...
public class Event_Vtx extends Base{
    float x; float y; float z; float likelihood;
 

	public int ChunkId() {	return 12500; }
	public boolean isCompactible() {return true;}    

	public String toString() {
		return String.format("Event_Vtx [x=%s, y=%s, z=%s, likelihood=%f]", x, y, z, likelihood);
	}
	...
	public int readData(URL url, DataInputStream din, Base obj, Base parent, int bytesToRead, int fileOffset, int level) throws IOException {
		...
		likelihood 	= Factory.getFloat (din, BytesRead);
		return BytesRead[0];
	}
	...
	public ByteBuffer writeData(){		
		...
		Factory.putFloat (toReturn, likelihood);				
		return toReturn;		
	}
