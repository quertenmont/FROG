...
public class Prim_Disk extends Base {
	float Radius;
	float PosX;    float PosY;  float PosZ;					
	float DirX;	   float DirY;	float DirZ;	
	int NPhi;	
		
	public int ChunkId() {	return 42030; }
	public boolean isCompactible() {return true;}	

	public String toString() {
		return "Prim_Disk [Radius=" + Radius + ", PosX=" + PosX + ", PosY="
	...
	public void draw(GL2 gl) {
		if(VBO_Vertices[0]==0){										
			float [] positionData   = new float[(3+3)*3*(int)NPhi];	
			...
			SetNormalArray(positionData, CubeIndex, 0, (3+3)*NPhi, 3, normalData);
			...
			if(jfrog.Common.support_VBO){					
				...
				gl.glDrawElements( GL2.GL_TRIANGLES, (3+3)*NPhi, GL2.GL_UNSIGNED_INT, CubeIndexBuf);
				gl.glEndList();			
			}
			...
			if(jfrog.Common.support_VBO){
				...
				gl.glDrawElements( GL2.GL_TRIANGLES    , (3+3)*NPhi, GL2.GL_UNSIGNED_INT, 0);				
			}else{			
